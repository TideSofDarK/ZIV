...
if testCount == nil then
  if not enigma then
    enigma = CreateUnitByName('npc_dota_creep_badguys_melee', Vector(0,0,0), true, hero, hero, hero:GetTeamNumber())
    enigma:SetModel('models/heroes/enigma/enigma.vmdl')
    enigma:SetOriginalModel('models/heroes/enigma/enigma.vmdl')

    Physics:Unit(enigma)

    planet1 = CreateUnitByName('npc_dota_creep_badguys_melee', Vector(0,0,0), true, hero, hero, hero:GetTeamNumber())
    ...
    planet2 = CreateUnitByName('npc_dota_creep_badguys_melee', Vector(0,0,0), true, hero, hero, hero:GetTeamNumber())
    ...
    planet3 = CreateUnitByName('npc_dota_creep_badguys_melee', Vector(0,0,0), true, hero, hero, hero:GetTeamNumber())
  ...
  Timers:CreateTimer(function()
    ...
    collider.test = function(self, collider, collided)
      return IsPhysicsUnit(collided) and collided.GetUnitName and collided:GetUnitName() == "npc_dota_creep_badguys_melee"
    end
...
if testCount == 0 then
  ...
  if testUnit == nil then
    ...
    testUnit = CreateUnitByName('npc_dota_creep_badguys_melee', hero:GetAbsOrigin(), true, hero, hero, hero:GetTeamNumber())
  ...
  if testUnit2 == nil then
    --PrecacheUnitByNameAsync("npc_dota_hero_slark", function(...) end)
    testUnit2 = CreateUnitByName('npc_dota_creep_badguys_melee', hero:GetAbsOrigin(), true, hero, hero, hero:GetTeamNumber())
  ...
  units = {}
  --[[for i=1,4 do
    units[i] = CreateUnitByName('npc_dota_creep_badguys_melee', hero:GetAbsOrigin(), true, hero, hero, hero:GetTeamNumber())
    --units[i]:FindAbilityByName("ziv_dummy_unit"):SetLevel(1)
...
if testCount == 3 then
  ...
  Timers:CreateTimer("timer", {
    callback = function()
      local unit = CreateUnitByName('npc_dota_creep_badguys_melee', hero:GetAbsOrigin() + hero:GetForwardVector() * 100, true, hero, hero, hero:GetTeamNumber())
      ...
      projCollider.test = function(self, collider, collided)
        return IsPhysicsUnit(collided) and collided.GetUnitName ~= nil and collided:GetUnitName() == "npc_dota_creep_badguys_melee"
      end
