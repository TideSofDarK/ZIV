...
function Crafting:UsePart( item, count, pID )
	...
	if item:GetCurrentCharges() == 0 then
		ZIV.pidInventory[pID]:RemoveItem(item)
	end
end
...
function Crafting:CraftingRequest( keys )
	...
	for k,v in pairs(recipe) do
		if ZIV.pidInventory[pID]:CheckForItem(k, v) == false then
			pass = false
			return false
		end
	end
	...
	for k,v in pairs(recipe) do
		local item = ZIV.pidInventory[pID]:GetItemsByName(k)[1]
	...
	local item = CreateItem(item_name, PlayerResource:GetPlayer(pID):GetAssignedHero(), PlayerResource:GetPlayer(pID):GetAssignedHero())
  	ZIV.pidInventory[pID]:AddItem(item)
