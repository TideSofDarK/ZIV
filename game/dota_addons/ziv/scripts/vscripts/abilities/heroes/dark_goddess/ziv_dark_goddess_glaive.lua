...
function SpawnGlaive( keys )
	...
	CreateUnitByNameAsync("npc_dummy_unit",caster:GetAttachmentOrigin(caster:ScriptLookupAttachment("attach_mom_r")),false,caster,caster,caster:GetTeamNumber(), function ( glaive )
		...
		glaive:OnPhysicsFrame(function(unit)
			...
			elseif state == GLAIVE_STATE_SUSTAIN then
				...
				if distance:Length2D() < 25 or time > max_point_time then
				    ...
				    TimedEffect( "particles/heroes/dark_goddess/dark_goddess_glaive_alt_d.vpcf", caster, 5.0, 5, PATTACH_POINT_FOLLOW )
