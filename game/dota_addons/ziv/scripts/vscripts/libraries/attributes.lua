...
function Attributes:ModifyBonuses(hero)
    ...
    Timers:CreateTimer(function()
        ...
        if strength ~= hero.strength then
            ...
            -- HP Bonus
            if not hero:HasModifier("modifier_health_bonus_fix") then
                Attributes.applier:ApplyDataDrivenModifier(hero, hero, "modifier_health_bonus_fix", {})
            end

            local health_stacks = math.abs(strength * Attributes.hp_adjustment)
            hero:SetModifierStackCount("modifier_health_bonus_fix", Attributes.applier, health_stacks)

            -- HP Regen Bonus
            if not hero:HasModifier("modifier_health_regen_constant_fix") then
                Attributes.applier:ApplyDataDrivenModifier(hero, hero, "modifier_health_regen_constant_fix", {})
            end

            local health_regen_stacks = math.abs(strength * Attributes.hp_regen_adjustment * 100)
            hero:SetModifierStackCount("modifier_health_regen_constant_fix", Attributes.applier, health_regen_stacks)
        end
        ...
        if agility ~= hero.agility then        
            ...
            -- Attack Speed Bonus
            if not hero:HasModifier("modifier_attackspeed_bonus_constant_fix") then
                Attributes.applier:ApplyDataDrivenModifier(hero, hero, "modifier_attackspeed_bonus_constant_fix", {})
            end

            local attackspeed_stacks = math.abs(agility * Attributes.attackspeed_adjustment)
            hero:SetModifierStackCount("modifier_attackspeed_bonus_constant_fix", Attributes.applier, attackspeed_stacks)
        end
        ...
        if intellect ~= hero.intellect then
            ...
            -- Mana Bonus
            if not hero:HasModifier("modifier_mana_bonus_fix") then
                Attributes.applier:ApplyDataDrivenModifier(hero, hero, "modifier_mana_bonus_fix", {})
            end

            local mana_stacks = math.abs(intellect * Attributes.mana_adjustment)
            hero:SetModifierStackCount("modifier_mana_bonus_fix", Attributes.applier, mana_stacks)

            -- Mana Regen Bonus
            if not hero:HasModifier("modifier_base_mana_regen_fix") then
                Attributes.applier:ApplyDataDrivenModifier(hero, hero, "modifier_base_mana_regen_fix", {})
            end
