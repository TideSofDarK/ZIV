...
function OnRightButtonPressed( nMouseButton )
{
	...
	if(Entities.GetAbilityCount(Players.GetLocalPlayerPortraitUnit()) > 6){
		castAbilityIndex = Entities.GetAbility( Players.GetPlayerHeroEntityIndex( Players.GetLocalPlayer() ), 6 );
	}
	if(Entities.IsSilenced(player)){
		if(Abilities.GetAbilityName(castAbilityIndex) == "berserker_whirlwind"){
			BeginMoveState();
			return;
		}
	}
	...
	if ( targetIndex === -1 )
	{
		BeginSpellState( 1, castAbilityIndex, -1 );
	}
	else if ( Entities.IsItemPhysical( targetIndex ) )
	{
		BeginPickUpState( targetIndex );
	...
	{
		BeginSpellState( 1, castAbilityIndex, targetIndex );
	}
